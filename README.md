...
- **Hibernation works under heavy game load.** On this UMA APU, hibernating with a big game resident used to fail (`amdgpu ... -ENOMEM` while evicting GPU buffers) and could hard-hang. The plugin now shrinks the hibernation image (`/sys/power/image_size=0`, set in the resume hook) and offers an opt-in **"Disable zram"** toggle — together these free enough physical RAM for the GPU eviction to succeed (verified hibernating from *No Man's Sky*).
